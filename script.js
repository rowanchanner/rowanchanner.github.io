...
async function loadPopularRow() {
  ...
  try {
    ...
    if (data.results) {
      renderToRow(popularRow, data.results, { forceType: "movie" });
    }
  } catch (err) {
    console.error("Popular row error:", err);
  }
}
...
async function loadNewReleasesRow() {
  ...
  try {
    ...
    if (data.results) {
      renderToRow(newReleasesRow, data.results, { forceType: "movie" });
    }
  } catch (err) {
...
async function loadMoviesRow() {
  ...
  try {
    ...
    if (data.results) {
      renderToRow(moviesRow, data.results, { forceType: "movie" });
    }
  } catch (err) {
    console.error("Movies row error:", err);
  }
}
...
async function loadTVRow() {
  ...
  try {
    ...
    console.error("TV row error:", err);
  }
}
...
window.addEventListener("scroll", () => {
  ...
  if (window.innerHeight + window.scrollY >= document.body.scrollHeight - 200) {
    if (trendingPage <= MAX_TRENDING_PAGES) {
      loadTrending();
    } else if (discoverPage <= MAX_DISCOVER_PAGES) {
      loadDiscover();
    }
  }
});
...
function setContinueWatchingList(list) {
...
function saveContinueWatching(movie) {
  try {
    ...
    list = list.filter((m) => !(m.id === movie.id && (m.type || m.media_type) === (movie.media_type || movie.type || "movie")));
...
function removeContinueWatchingItem(id, type) {
  let list = getContinueWatchingList();
...
function loadContinueWatching() {
  ...
  list.forEach((item) => {
    ...
    cardWrap.className = "continue-card-wrap";
    ...
    a.setAttribute("aria-label", `Continue watching ${item.title}`);

    a.innerHTML = `
      <div class="continue-overlay">${item.title}</div>
    `;
    ...
    removeBtn.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      removeContinueWatchingItem(item.id, item.type);
    });

    cardWrap.appendChild(a);
    cardWrap.appendChild(removeBtn);
    continueGrid.appendChild(cardWrap);
  });

  if (continueGrid.children.length === 0) {
    continueSection.classList.add("hidden");
  }
}
...
function setupRowScrollButtons() {
  document.querySelectorAll("[data-scroll-left]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (row) {
        row.scrollBy({
          left: -Math.max(320, row.clientWidth * 0.85),
          behavior: "smooth"
        });
      }
    });
